...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = from e in emps from s in grades 
                where e.Sal >= s.Losal && e.Sal <= s.Hisal select new {e.EName, s.Grade};
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = from e in emps let avgSal = emps.Where(d => d.DeptNo == e.DeptNo)
                    .Average(d => d.Sal) where e.Sal > avgSal select e.EName;
